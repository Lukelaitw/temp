...
def resolve_vae_checkpoint_paths(ckpt_cfg: Any) -> tuple[Path, Path]:
    """Resolve paths to the VAE weights file and its companion YAML config on disk.

    Default layout matches Lightning runs: ``{ckpt_path}/{job_name}/last.ckpt`` and
    ``config.yaml``. Set ``ckpt_filename`` / ``config_filename`` for flat artifacts
    (e.g. ``20M.ckpt`` and ``20M.yaml`` in ``ckpt_path`` with empty ``job_name``).
    """
    ckpt_root = Path(str(ckpt_cfg.ckpt_path))
    job_name = OmegaConf.select(ckpt_cfg, "job_name")
    if job_name not in (None, ""):
        job_path = ckpt_root / str(job_name).strip("/")
    else:
        job_path = ckpt_root

    ckpt_override = OmegaConf.select(ckpt_cfg, "ckpt_filename")
    epoch = OmegaConf.select(ckpt_cfg, "epoch")
    if ckpt_override not in (None, ""):
        ckpt_name = str(ckpt_override)
    elif epoch is not None and isinstance(epoch, int):
        ckpt_name = f"epoch={epoch}.ckpt"
    else:
        ckpt_name = "last.ckpt"

    ckpt_file = Path(ckpt_name) if Path(ckpt_name).is_absolute() else job_path / ckpt_name

    cfg_override = OmegaConf.select(ckpt_cfg, "config_filename")
    config_name = str(cfg_override) if cfg_override not in (None, "") else "config.yaml"
    config_file = Path(config_name) if Path(config_name).is_absolute() else job_path / config_name
